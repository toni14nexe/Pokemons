...
let serverHost = "https://api.jsonserver.io/users"
...
export const useUsersStore = defineStore("users", {
	...
	actions: {
		...
		async login(userData) {
			try {
				...
				if (userOK) {
					...
					$cookies.set(
						"user",
						{ role: role, username: userData.username, id: id },
						60 * 30
					)
				}
				return userOK
		...
		async addPokemonToFavourite(pokemon) {
			...
			try {
				let response = await this.getUser(userCookie.id)
				response.data.pokedex = this.replacePokemon(
